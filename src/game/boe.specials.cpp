...
void general_spec(const runtime_state& ctx) {
	...
	switch(cur_node.type) {
		...
		case eSpecType::TITLED_MSG:
			get_scen_str(str1, ctx.cur_spec_type, cur_node.m3);
			handle_message(ctx, str1, cur_node.pic, ePicType(cur_node.pictype));
			break;
		case eSpecType::DISPLAY_SM_MSG:
			get_scen_strs(str1, str2, ctx.cur_spec_type, cur_node.m1,cur_node.m2);
			if(cur_node.m1 >= 0)
		...
		case eSpecType::DISPLAY_PICTURE:
			...
			check_mess = false;
			get_scen_str(str1, ctx.cur_spec_type, spec.m1);
		...
		case eSpecType::APPEND_STRING:
			get_scen_str(str1,ctx.cur_spec_type,spec.ex1a);
...
void oneshot_spec(const runtime_state& ctx) {
	...
	switch(cur_node.type) {
		...
		case eSpecType::ONCE_DIALOG:
			...
			if(spec.m1 < 0)
				break;
			get_scen_strs(strs, ctx.cur_spec_type, spec.m1);
			if(spec.m3 > 0) {
				buttons[0] = 1;
				buttons[1] = spec.ex1a;
				buttons[2] = spec.ex2a;
		...
		case eSpecType::ONCE_TRAP:
			...
			if((spec.m1 >= 0) || (spec.m2 >= 0)) {
				get_scen_strs(strs[0],strs[1], ctx.cur_spec_type, spec.m1, spec.m2);
...
void affect_spec(const runtime_state& ctx) {
	...
	switch(cur_node.type) {
		...
		case eSpecType::AFFECT_NAME:
			get_scen_str(str, ctx.cur_spec_type, spec.m3);
...
void ifthen_spec(const runtime_state& ctx) {
	...
	switch(cur_node.type) {
		...
		case eSpecType::IF_TEXT_RESPONSE:
			check_mess = false;
			get_scen_str(str1,eSpecCtxType::SCEN,spec.m1);
			str3 = get_text_response(str1);
			spec.pic = minmax(0,50,spec.pic);
			get_scen_strs(str1,str2,eSpecCtxType::SCEN,spec.ex1a,spec.ex2a);
...
void townmode_spec(const runtime_state& ctx) {
	...
	switch(cur_node.type) {
		...
		case eSpecType::TOWN_PLACE_LABEL:
			...
			get_scen_strs(strs[0], strs[1], ctx.cur_spec_type, spec.m1, spec.m1);
...
void handle_message(const runtime_state& ctx, const std::string& title, pic_num_t pic, ePicType pt) {
	...
	std::string str1, str2;
	get_scen_strs(str1, str2, ctx.cur_spec_type, ctx.cur_spec.m1, ctx.cur_spec.m2);
...
bool get_scen_str(std::string& str,eSpecCtxType cur_type,short which_str) {
...
void get_scen_strs(std::string& str1,std::string& str2,eSpecCtxType cur_type,short which_str1,short which_str2) {
	get_scen_str(str1, cur_type, which_str1);
	get_scen_str(str2, cur_type, which_str2);
}

void get_scen_strs(std::array<std::string,6>& strs,eSpecCtxType cur_type,short which_str1) {
	for(int i = 0; i < 6; ++i)
		if (!get_scen_str(strs[i],cur_type, which_str1 + i)) return;
}
