...
std::map<sf::RenderTexture*,sf::RenderTexture*> store_scale_aware_text;
...
static void draw_stored_scale_aware_text(sf::RenderTexture& texture, sf::RenderTarget& dest_window, rectangle targ_rect) {
	...
	sf::RenderTexture* stored_text = store_scale_aware_text[&texture];
	sf::Sprite text(stored_text->getTexture());
	sf::Vector2f position = {0.0f+targ_rect.left, 0.0f+targ_rect.top};
	position *= (float)get_ui_scale();
	position += scaled_top_left;
	dest_window.draw(text);
